...
export async function incrementView(postId) {
  // Per spec: GET /posts/{postId}?increaseClick=true triggers clickCount increment
  try {
    const data = await apiGet(`/posts/${postId}?increaseClick=true`);
    if (data) return mapPost(data);
    return null;
  } catch (err) {
    console.warn(
      `[posts] failed to increment view for ${postId}:`,
      err?.message || err
    );
    return null;
  }
...
const postsApi = { list, create, incrementView };
